...
class TrainTestCase(unittest.TestCase):
    ...
    def test_memory(self):
        ...
        # Get the shape
        result = ((linear.in_features * linear.out_features)
                  * (4 / (1024 ^ 3)))*2
        print(result)
        train_processor.trainIters(1, None)
        train_processor.trainIters(1, None)
    ...
    def test_shape(self):
        linear = nn.Linear(2, 4, bias=True)
    ...
    def test_max_pool(self):
        ...
        for x in range(answer_index.size(0)):
            ...
            for item in torch.unique(answer_index[x, :]):
                ...
                result.append(torch.masked_select(
                    encoder_outputs, (answer_index == item)).sum().item())
